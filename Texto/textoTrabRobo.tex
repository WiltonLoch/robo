...
%
%
% Ainda no grid, a representação de um espaço vazio é dada pelo número zero em uma determinada célula, enquanto a existência de um item naquelas coordenadas é dada pelo número um. Os agentes são representados por objetos instanciados de uma classe que descreve as formigas e estes atuam sobre o \textit{grid} de modo a movimentar os itens da forma especificada. A movimentação das formigas é puramente randômica. Foi testada também uma forma de movimentação baseada em probabilidades distintas de movimentação perante a existência ou não de itens nas possíveis direções de caminhada, porém esta abordagem não obteve vantagens experimentais sobre a anterior nem afetou positivamente a convergência da solução. Não foi implementada nenhuma forma de memória para os agentes, desta forma eles simplesmente caminham pelo \textit{grid} enquanto tratam probabilidades de pegar e de soltar os itens.
%
